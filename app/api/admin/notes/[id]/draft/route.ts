...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(await saveDraft(prisma, { contentType: "NOTE", documentId: (await params).id, actorId: admin.user.id, payload: (await request.json())?.payload }), { status: 201 });
  } catch (error) {
