...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const content = await getAdminContent(prisma, "PROJECT", id);
    ...
    const token = createPreviewToken({ contentType: "PROJECT", documentId: id, revisionId });
    return NextResponse.json({ url: `/preview/project/${encodeURIComponent(id)}?token=${encodeURIComponent(token)}`, expiresInSeconds: 900 });
