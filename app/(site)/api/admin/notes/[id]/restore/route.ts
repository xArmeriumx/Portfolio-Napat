...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await restoreRevision(prisma, { contentType: "NOTE", documentId: (await params).id, revisionId: body.revisionId, actorId: admin.user.id }), { status: 201 });
  } catch (error) {
