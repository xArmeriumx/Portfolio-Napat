...
export async function GET() {
  ...
  try { return NextResponse.json(await listAdminContent(prisma, "PROJECT")); } catch (error) { return adminErrorResponse(error); }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(await createContentDraft(prisma, { contentType: "PROJECT", actorId: admin.user.id, payload: body?.payload }), { status: 201 });
