    ...
    <section className="min-h-screen bg-[#f9fafb] px-4 pb-24 pt-32 md:px-6">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 space-y-6">
          ...
          <fieldset className="rounded-2xl border border-gray-200 bg-white p-6"><legend className="px-2 text-lg font-black text-gray-900">SEO overrides</legend><p className="mb-4 text-sm text-gray-500">เว้นว่างเพื่อใช้ค่า SEO ที่ระบบสร้างจาก Published Profile</p><div className="grid gap-4 md:grid-cols-2"><TextField label="SEO title EN" value={localized(payload.seo?.title, "en")} onChange={(value) => updateSeo("title", "en", value)} /><TextField label="SEO title TH" value={localized(payload.seo?.title, "th")} onChange={(value) => updateSeo("title", "th", value)} /><TextArea label="SEO description EN" value={localized(payload.seo?.description, "en")} onChange={(value) => updateSeo("description", "en", value)} /><TextArea label="SEO description TH" value={localized(payload.seo?.description, "th")} onChange={(value) => updateSeo("description", "th", value)} /><TextField label="SEO image path/URL" value={payload.seo?.image} onChange={(value) => update(["seo", "image"], value || null)} /></div></fieldset>
