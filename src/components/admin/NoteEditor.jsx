...
export default function NoteEditor({ documentId = null, initialPayload, draftRevisionId: initialDraftRevisionId = null, revisions = [] }) {
  ...
  async function preview(locale = "en") {
    setBusy(true); setError("");
    try {
      const result = await call(`/api/admin/notes/${documentId}/preview`, { revisionId: draftRevisionId });
      window.open(`${result.url}&locale=${locale}`, "_blank", "noopener,noreferrer");
    ...
    <section className="min-h-screen bg-[#f9fafb] px-4 pb-24 pt-32 md:px-6">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 space-y-6">
          ...
          <fieldset className="space-y-4 rounded-2xl border border-gray-200 bg-white p-6"><legend className="px-2 text-lg font-black">Translations / ฉบับแปล</legend>
            <p className="text-sm text-gray-600">ใส่เนื้อหาแต่ละภาษาที่ตรวจแล้ว ช่องว่างจะใช้ต้นฉบับและไม่เข้าสู่ดัชนีค้นหา บันทึก Draft ก่อน Preview</p>
            {["en", "th"].map((locale) => <div key={locale}><p className="text-sm font-bold">{locale.toUpperCase()}: {payload.bodyMarkdownByLocale?.[locale]?.trim() && payload.title[locale]?.trim() ? "พร้อม Preview / Ready for review" : "ยังไม่ครบ / Incomplete"}</p><Field multiline rows={18} label={`Body ${locale.toUpperCase()}`} value={payload.bodyMarkdownByLocale?.[locale] || ""} onChange={(value) => update(["bodyMarkdownByLocale"], { ...payload.bodyMarkdownByLocale, [locale]: value })} /></div>)}
          </fieldset>
