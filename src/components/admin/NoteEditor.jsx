...
export default function NoteEditor({ documentId = null, initialPayload, draftRevisionId: initialDraftRevisionId = null }) {
  ...
  return <section className="min-h-screen bg-[#f9fafb] px-4 pb-24 pt-32 md:px-6"><div className="mx-auto max-w-5xl"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#c43c3c]">Notes / Draft Editor</p><h1 className="mt-3 text-4xl font-black text-gray-900">{documentId ? "Edit Note" : "New Note"}</h1><p className="mt-3 text-sm text-gray-500">Markdown จะไม่เปิด raw HTML และ URL scheme ที่รันโค้ดได้</p></div><a href="/admin/notes" className="text-sm font-bold text-gray-500">← Notes</a></div><div className="mt-8 flex flex-wrap gap-3"><button disabled={busy} onClick={save} className="rounded-full bg-gray-900 px-5 py-3 text-sm font-black text-white disabled:opacity-50">Save Draft</button><button disabled={busy || !documentId || !draftRevisionId} onClick={preview} className="rounded-full border border-gray-300 bg-white px-5 py-3 text-sm font-black disabled:opacity-50">Preview exact Draft</button><button disabled={busy || !documentId || !draftRevisionId} onClick={publish} className="rounded-full bg-[#c43c3c] px-5 py-3 text-sm font-black text-white disabled:opacity-50">Publish</button>{documentId && <button disabled={busy} onClick={archive} className="rounded-full border border-red-200 bg-red-50 px-5 py-3 text-sm font-black text-red-700 disabled:opacity-50">Archive</button>}</div>{message && <p role="status" className="mt-4 rounded-xl bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">{message}</p>}{error && <p role="alert" className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{error}</p>}<div className="mt-8 space-y-6"><fieldset className="rounded-2xl border border-gray-200 bg-white p-6"><legend className="px-2 text-lg font-black">Identity / ตัวตน</legend><div className="grid gap-4 md:grid-cols-2"><Field label="Slug" value={payload.slug} onChange={(value) => update(["slug"], value)} /><Field label="Display order" type="number" value={payload.order} onChange={(value) => update(["order"], Number(value))} /><Field label="Title EN" value={payload.title.en} onChange={(value) => update(["title", "en"], value)} /><Field label="Title TH" value={payload.title.th} onChange={(value) => update(["title", "th"], value)} /><Field label="Raw filename" value={payload.rawName} onChange={(value) => update(["rawName"], value)} /><Field multiline label="Excerpt EN" value={payload.excerpt.en} onChange={(value) => update(["excerpt", "en"], value)} /><Field multiline label="Excerpt TH" value={payload.excerpt.th} onChange={(value) => update(["excerpt", "th"], value)} /></div></fieldset><fieldset className="rounded-2xl border border-gray-200 bg-white p-6"><legend className="px-2 text-lg font-black">Markdown body</legend><Field multiline rows={28} label="Body Markdown" value={payload.bodyMarkdown} onChange={(value) => update(["bodyMarkdown"], value)} /></fieldset><fieldset className="rounded-2xl border border-gray-200 bg-white p-6"><legend className="px-2 text-lg font-black">SEO</legend><div className="grid gap-4 md:grid-cols-2"><Field label="SEO title EN" value={payload.seo.title?.en || ""} onChange={(value) => updateSeo("title", "en", value)} /><Field label="SEO title TH" value={payload.seo.title?.th || ""} onChange={(value) => updateSeo("title", "th", value)} /><Field multiline label="SEO description EN" value={payload.seo.description?.en || ""} onChange={(value) => updateSeo("description", "en", value)} /><Field multiline label="SEO description TH" value={payload.seo.description?.th || ""} onChange={(value) => updateSeo("description", "th", value)} /></div></fieldset></div></div></section>;
}
